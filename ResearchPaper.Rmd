...
As we can see from the fitted values above, the inclusion of a more distributed set of control variables did not change our results significantly and the explanatory power remains low. In a next step, we therefore substitute individual crime dummies for our categories of crimes. 
...
```{r Map Prostitution and Gambling crime, echo=FALSE, warnings = FALSE, error=FALSE}
## Clustered crimes 
...
coltable9 <- c('PerCapitaIncome', '16+Unemployed%','HouseholdsPoverty%','25+WithoutHighSchool%', 'HousingCrowded%', 'Under18Over64%', 'Arson', 'predicted')
fitted3$predicted <- predict(Arson_probit3, newdata = fitted3, type = 'response', interval='confidence')
kable(fitted3, align = 'c', digits = 3, caption = 'Predicted Probabilities for Fitted Values', col.names=coltable9)
...
coltable10 <- c('PerCapitaIncome', '16+Unemployed%','HouseholdsPoverty%','25+WithoutHighSchool%', 'HousingCrowded%', 'Under18Over64%', 'Violent', 'predicted')
kable(fitted3, align = 'c', digits = 3, caption = 'Predicted Probabilities for Fitted Values', col.names=coltable10)
...
coltable11 <- c('PerCapitaIncome', '16+Unemployed%','HouseholdsPoverty%','25+WithoutHighSchool%', 'HousingCrowded%', 'Under18Over64%', 'Property', 'predicted')
kable(fitted3, align = 'c', digits = 3, caption = 'Predicted Probabilities for Fitted Values', col.names=coltable11)
...
coltable12 <- c('PerCapitaIncome', '16+Unemployed%','HouseholdsPoverty%','25+WithoutHighSchool%', 'HousingCrowded%', 'Under18Over64%', 'Property', 'predicted')
kable(fitted3, align = 'c', digits = 3, caption = 'Predicted Probabilities for Fitted Values', col.names=coltable12)
...
```{r Linear Regression Relative Distance, echo=FALSE}

## Linear Models with Relative Distances 

Violent_linear2 <- lm(rel_dist_sta ~ as.integer(hardship_index) + as.factor(violent), data=Crime_Data )

Property_linear2 <- lm(rel_dist_sta ~ as.integer(hardship_index) + as.factor(property), data=Crime_Data )

Money_linear2 <- lm(rel_dist_sta ~ as.integer(hardship_index) + as.factor(money), data=Crime_Data )

```

```{r Linear Models Stargazer Rel Dist, echo=FALSE, results='asis'}
## Create covariate labels
labelsLinear1 <- c('Hardship Index', 'Violent Dummy', 'Property Dummy', 'Money Dummy')

##Using stargazer, we create a table that summarizes the results of the Models 5.1 to 5.3. 
stargazer::stargazer(Violent_linear2, Property_linear2, Money_linear2, covariate.labels = labelsLinear1,
    title = 'Linear Regression Estimates of Model 5.4 to 5.6',
    digits = 2, type = 'html')
...
Narcotics_linear2 <- lm(distance_sta ~ as.integer(hardship_index) + as.factor(Narcotics), data=Crime_Data )

Arson_linear2 <- lm(distance_sta ~ as.integer(hardship_index) + as.factor(Arson), data=Crime_Data )

```

```{r Linear Models Stargazer Ind. Files, echo=FALSE, results='asis'}
## Create covariate labels
labelsLinear2 <- c('Hardship Index', 'Prostitution Dummy', 'Gambling Dummy', 'Narcotics Dummy', 'Arson Dummy')

##Using stargazer, we create a table that summarizes the results of the Models 6.1 to 6.4. 
stargazer::stargazer(Prostitution_linear2, Gambling_linear2, Narcotics_linear2, Arson_linear2, covariate.labels = labelsLinear2,
    title = 'Linear Regression Estimates of Model 6.1 to 6.4',
    digits = 2, type = 'html')
...
```{r Linear Regression Relative Distance Ind. Crimes, echo=FALSE}

## Linear Reg for individual Crimes 

Prostitution_linear3 <- lm(rel_dist_sta ~ as.integer(hardship_index) + as.factor(Prostitution), data=Crime_Data )

Gambling_linear3 <- lm(rel_dist_sta ~ as.integer(hardship_index) + as.factor(Gambling), data=Crime_Data )

Narcotics_linear3 <- lm(rel_dist_sta ~ as.integer(hardship_index) + as.factor(Narcotics), data=Crime_Data )

Arson_linear3 <- lm(rel_dist_sta ~ as.integer(hardship_index) + as.factor(Arson), data=Crime_Data )

```

```{r Linear Models Stargazer Ind. Crimes Rel, echo=FALSE, results='asis'}
## Create covariate labels
labelsLinear2 <- c('Hardship Index', 'Prostitution Dummy', 'Gambling Dummy', 'Narcotics Dummy', 'Arson Dummy')

##Using stargazer, we create a table that summarizes the results of the Models 6.4 to 6.8. 
stargazer::stargazer(Prostitution_linear3, Gambling_linear3, Narcotics_linear3, Arson_linear3, covariate.labels = labelsLinear2,
    title = 'Linear Regression Estimates of Model 6.4 to 6.8',
    digits = 2, type = 'html')
